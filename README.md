...
**60°C (new temperature, Jul 2026):** no `60C.zip` bundling all samples yet — each sample gets its own zip, registered in `EXPERIMENTS_60C` (`cpmg_batch_fit.py`, mirrored in `pub_figs/cpmg_fit.py`):
...
Raw data for this dataset originates on the instrument PC's `Z:` share (not OneDrive — a prior OneDrive copy was found to overwrite file mtimes with the upload time, destroying the per-scan acquisition timestamps the whole α(t) analysis depends on; verify with `stat`/`find` on `data.2d` before zipping if re-copying this kind of dataset in the future). Adding DAP/DAB at 60°C later is just a new `EXPERIMENTS_60C` entry + zip, no further code changes.
...
**T2_0** increases with temperature as expected (more liquid-like uncured state). ~~EDA 40°C is anomalous (T2_0 ≈ 0.112 s, ~3× other samples at 40°C)~~ — **update (Jul 2026):** this was a fit bug, not a real effect. `_fit_echo_array`'s initial-guess logic (`cpmg_batch_fit.py`) computed the amplitude guess from `max()` over the *entire* echo array and located the half-max crossing via `np.searchsorted` on `-y`, which silently assumes `y` is sorted — invalid once real signal has decayed away and the rest of the array is noise. For EDA 40°C, a single late-time (132.9 min), fully-vitrified scan got a spurious, falsely-confident T2 ≈ 0.112 s fit that slipped past the drop criteria (β=1.50, under the β>2 cutoff) and became the `T2_0` reference (= max reliable T2) for the whole series, including producing a nonphysical α ≈ −2 point in the Corezzi fit. Fixed by (1) restricting the amplitude guess to the first 15% of points, and (2) finding the *first* index where `y` actually drops below half-max rather than using `searchsorted` on unsorted data, floored at half an echo spacing. EDA 40°C now reports T2_0 ≈ 0.037 s, consistent with the other 40°C samples. Re-running the full batch after this fix changed DAP 40°C's `B`/`a0` slightly (0.116→0.120, 1.017→1.017) for the same reason; all other datasets were affected only at the level of floating-point convergence noise (~1e-5 relative).
...
**NMR data source (Jul 2026):** α(t) for NMR datasets normally comes from `epoxy_data_13Mar2026.mat` via the `nmr_index` dict (`(sample, temp) → column`), built externally and not regenerated by anything in this repo. For `(sample, temp)` pairs not in `nmr_index` — currently just `('EDA', 60)` — `load_nmr_alpha_t()` falls back to reading `cpmg_fit_results/{sample}_{temp}C.csv` directly, converting `elapsed_min` to seconds to match the `.mat`'s convention. Existing datasets are unaffected; this is how a new dataset can be fit before it's folded into the `.mat`.
